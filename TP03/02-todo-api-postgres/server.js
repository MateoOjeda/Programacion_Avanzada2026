...
app.put('/tasks/:id', async (req, res) => {
 ...
 //Nota mia para no olvidar COALESCE significa, en este caso, que si el valor que 
 //se pasa es NULL, se mantiene el valor actual de la columna. Por ejemplo, 
 //si no se pasa un nuevo título, se mantiene el título actual de la tarea.

 //updated_at = CURRENT_TIMESTAMP lo que hace  es que actualiza el regitro de fecha y hora actual, 
 // para saber cuando se modifico la tarea.
...
app.delete('/tasks/:id', async (req, res) => {
    try {
        const { id } = req.params // Obtenengo el ID de la tarea que tengo que eliminar 

        const result = await pool.query( // Hago la consulta a la base de datos para eliminar la tarea con el ID especificado
            `DELETE FROM tasks WHERE id = $1 RETURNING *`,
            [id]
        )

        if (result.rows.length === 0) { // Si no se devuelve ninguna fila, significa que la tarea no existía, por lo que respondo con un error 404
            return res.status(404).json({ error: 'Task not found' })
        }

        res.status(204).end()   // Si la tarea se eliminó correctamente, respondo con un código 204 (sin contenido) y termino la respuesta
    } catch (error) {
        console.error(error)
        res.status(500).json({ error: 'Internal server error' })
    }
